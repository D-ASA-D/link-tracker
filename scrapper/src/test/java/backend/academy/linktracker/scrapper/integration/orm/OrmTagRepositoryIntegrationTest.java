...
class OrmTagRepositoryIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    @BeforeEach
    void init() {
        chatRepository.register(601L);
        LinkRecord link = linkRepository.save("https://github.com/test/orm-tag-link", Instant.now(), Instant.now());
        SubscriptionRecord subscription = subscriptionRepository.save(601L, link.id());
