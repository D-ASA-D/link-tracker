...
class TrackCommandTest {
    ...
    void shouldStartTrackingDialog() {
        ...
        TrackCommand command = new TrackCommand(sessionService, bot);
        ...
        assertThat(session.getState()).isEqualTo(UserState.WAITING_FOR_LINK);
        ArgumentCaptor<SendMessage> captor = ArgumentCaptor.forClass(SendMessage.class);
        verify(bot).execute(captor.capture());

        SendMessage sentMessage = captor.getValue();
        assertThat(sentMessage.getParameters().get("text")).isEqualTo("Отправьте ссылку для отслеживания.");
    }
}
